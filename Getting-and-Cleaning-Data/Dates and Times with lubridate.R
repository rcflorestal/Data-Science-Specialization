...
## day. We can extract any of these components using the year(), month(), or day()
...
## To see how these functions work, try ymd("1989-05-17")
my_date <- ymd("1989-05-17"); my_date

## t looks almost the same, except for the addition of a time zone, which we'll 
...
## What about to to "192012"? So that it's clear we want January 2, 1920.
ymd("1920/1/2")


## In addition to dates, we can parse date-times. Creates a date-time object 
## called dt1
#
...
## helpful when you need to parse an entire column of data. Creates a vector of
## dates called dt2.
#
dt2 <- c("2014-05-14", "2014-09-22", "2014-07-11"); dt2
...
## this_moment unless we reassign the result to this_moment. To see this, 
...
#
this_moment <- update(this_moment, hours = 16, minutes = 40, seconds = 49)
this_moment
...
## This time, however, we'll specify the time zone that we want: "America/New_York".
...
## days to nyc, so we can use the following expression: nyc + days(2). Give it 
...
## Use the same approach to add 15 hours and 50 minutes to the date-time 
## stored in depart. Store the result in a new variable called arrive.
#
...
#
## The with_tz() function returns a date-time as it would appear in another 
## time zone. Use ?with_tz to check out the documentation.
?with_tz
...
#
arrive <- with_tz(arrive, "Asia/Hong_Kong"); arrive
...
## Singapore on June 17, 2008. Naturally, you'd like to know exactly how long
## it has been.
#
...
## Pull up the documentation for interval(), which we'll use to explore 
## how much time has passed between arrive and last_time.
#
?interval

## Create an interval() that spans from last_time to arrive. 
